...
class Objek extends \Restserver\Libraries\REST_Controller
{
    ...
    public function simpan_post()
    {
        ...
        if ($is_valid_token['status'] === true) {
            ...
            $Output = $this->Objek_model->insert($POST);
            if ($Output) {
                $this->response($Output, REST_Controller::HTTP_OK);
            }else{
    ...
    public function ubah_put()
    {
        ...
        if ($is_valid_token['status'] === true) {
            $POST = json_decode($this->security->xss_clean($this->input->raw_input_stream), true);
            $Output = $this->Objek_model->update($POST);
            if ($Output) {
    ...
    public function Hapus_delete()
    {
        ...
        $is_valid_token = $this->authorization_token->validateToken();
        if ($is_valid_token['status'] === true) {
            $Output = $this->Objek_model->delete($this->uri->segment(3));
            if ($Output) {
                $this->response(true, REST_Controller::HTTP_OK);
            }else{
                $this->response(false, REST_Controller::HTTP_BAD_REQUEST);
