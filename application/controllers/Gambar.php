...
class Gambar extends \Restserver\Libraries\REST_Controller
{
    ...
    {
        $output = $this->Gambar_model->select();
        if ($output) {
            ...
            if (isset($POST['idkelulusan'])) {
                if (isset($POST['file'])) {
                    ...
                    if (unlink($dirFile)) {
                        ...
                        $POST['Berkas'] = $file;
                        $Output = $this->Gambar_model->update($POST);
                        if ($Output) {
                ...
                file_put_contents($file_dir, $item['file']);
                $POST['Berkas'] = $file;
                $Output = $this->Gambar_model->insert($POST);
                if ($Output) {
                    $this->response($Output, REST_Controller::HTTP_OK);
                } else {
                    $this->response(false, REST_Controller::HTTP_BAD_REQUEST);
    ...
    public function ubah_put()
    {
        ...
        if ($is_valid_token['status'] === true) {
            $POST = json_decode($this->security->xss_clean($this->input->raw_input_stream), true);
            $datakelulusan = $this->Gambar_model->select($POST['idkelulusan']);
            $Output = $this->Gambar_model->update($POST);
            if ($Output) {
    ...
    public function Hapus_delete()
    {
        ...
        $is_valid_token = $this->authorization_token->validateToken();
        if ($is_valid_token['status'] === true) {
            $Output = $this->Gambar_model->delete($this->uri->segment(3));
            if ($Output) {
                $message = [
                    'status' => true,
                ];
